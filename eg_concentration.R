...
# ── Step 1: Load data (or generate pseudo data if .dta not found) ─────────────
# Attempt to read the real .dta file; fall back to synthetic data for testing.
dta_path <- "your_data.dta"   # <-- replace with actual path to your .dta file

raw <- tryCatch(
  read_dta(dta_path),          # read_dta(): reads Stata .dta files into a tibble
  error = function(e) {
...
# Mimics the panel structure: year × cityid × ind_3f cells with counts/capital.
if (is.null(raw)) {
  set.seed(42)
  ...
  raw <- expand_grid(                        # expand_grid(): creates all combinations
    year   = 2015:(2015 + n_years - 1),
    cityid = 1:n_cities,
    ind_3f = ind_codes
  ) |>
    mutate(
      # Introduce heterogeneity: some industries cluster in a few cities
      cluster_pull = ifelse(
        ind_3f == 364,
        ifelse(cityid <= 5, 8, 1),   # ind 364 concentrated in first 5 cities
        ifelse(cityid <= 3, 3, 1)
      ),
      stock_num     = rpois(n(), lambda = cluster_pull * 10) + 1L,
      stock_capital = round(stock_num * runif(n(), 500, 5000), 0)
    ) |>
    select(year, cityid, ind_3f, stock_num, stock_capital)
}

# ── Step 3: Collapse across years ─────────────────────────────────────────────
# Sum firm counts and capital over all years; EG is a cross-sectional index.
panel <- raw |>
  filter(!is.na(stock_num), !is.na(stock_capital)) |>   # filter(): removes NA rows
  group_by(cityid, ind_3f) |>
...
city_totals <- panel |>
  group_by(cityid) |>
  summarise(
    city_firms   = sum(firms),
    city_capital = sum(capital),
    .groups      = "drop"
  ) |>
  mutate(
    x_firms   = city_firms   / sum(city_firms),    # x_i for the firm-count index
    x_capital = city_capital / sum(city_capital)   # x_i for the capital index
  )

# Scalar denominators for G: 1 - Σ x_i²
denom_firms   <- 1 - sum(city_totals$x_firms^2)
denom_capital <- 1 - sum(city_totals$x_capital^2)
...
ind_city <- panel |>
  group_by(ind_3f) |>
  mutate(
    ind_total_firms   = sum(firms),
    ind_total_capital = sum(capital)
  ) |>
  ungroup() |>
  mutate(
    s_firms   = firms   / ind_total_firms,    # s_i: industry city share (firms)
    s_capital = capital / ind_total_capital   # s_i: industry city share (capital)
  ) |>
  left_join(city_totals, by = "cityid")       # left_join(): merges city x_i columns
